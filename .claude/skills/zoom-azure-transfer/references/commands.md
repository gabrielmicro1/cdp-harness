...
| probe `listing_ok: false` (rc 2), token_ok true | The S2S app was created but never ACTIVATED, or lacks the recording scope | Client conversation: activate the app on its Activation tab / add the scope. The #1 stall. NOT a retry. Read the error BODY — Zoom names the exact missing scope. |
| ...and the body says `does not contain scopes:[cloud_recording:read:list_account_recordings:admin]` (gr0, 2026-08-31) | The app is on Zoom's GRANULAR scopes, where the bundled classic `recording:read:admin` isn't offered at all — token mints fine, listing is unscoped | Client adds the Cloud Recording read scopes (`cloud_recording:read:list_account_recordings:admin` for the listing + `cloud_recording:read:list_recording_files:admin` for per-meeting re-resolve) and RE-ACTIVATES the app. `/users/me` + `/users/me/settings` are already guarded, so no user scope is required. |

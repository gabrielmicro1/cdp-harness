...
`--checkers`. Exit codes: 0 ok, 1 hard error, 2 refusal / check-failed
...
# throttle: --tpslimit 12 and modest parallelism, or Dropbox 429s.
rclone copy 'dropbox:<path>' azure:<slug>-raw/dropbox-export \
  ...
  --stats 1m --stats-log-level NOTICE --tpslimit 12

# Manual rescue on the VM
ssh azureuser@<ip>
tmux attach -t transfer          # watch live (detach: Ctrl-b d)
tail -f ~/transfer.log
rclone check 'dropbox:<path>' azure:<slug>-raw/dropbox-export --one-way
```
...
| Constant `too_many_requests` / 429 | Dropbox rate limiting | Expected in small doses (rclone retries). A wall of them: lower `--transfers` (e.g. 4) — do NOT raise tpslimit. |
| Throughput ~100–300 MiB/s, no errors | Normal Dropbox pace (API-bound, not pipe-bound) | Don't chase it; unlike GCS you won't see GiB/s. |

...
`--sas-days`, `--transfers`, `--checkers`. Exit codes: 0 ok, 1 hard error,
...
rclone copy 'gdrive:<path>' azure:<slug>-raw/gdrive-export \
  ...
  --stats 1m --stats-log-level NOTICE --tpslimit 10
...
| `invalid_grant` / `unauthorized` on `gdrive:` | Token revoked or from the wrong Google account | Fresh `rclone authorize "drive"` from the owner; re-run `write-gdrive-remote`. |
...
| `userRateLimitExceeded` / 403 bursts | Drive API rate limiting | Expected in small doses (rclone pacer retries). A wall of them: lower `--transfers` (e.g. 4) — never raise tpslimit. |
